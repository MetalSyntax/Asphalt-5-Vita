...
void soloader_init_all() {
    // Opened first so the log file captures the whole boot, including the
    // checks below that can end in fatal_error().
    if (log_init() > 0)
        l_success("Log #%03d opened at %s", log_current_index(),
                  log_current_path());
